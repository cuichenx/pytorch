        ...
        if V.graph.cpp_wrapper:
            ...
            if self.setup_stream_cache:
                ...
                if self.num_streams > 1:
                    ...
                    if V.graph.device_type == "cuda" and (
                        default_indices or new_indices or event_indices
                    ):
                        code.writeline(
                            "from torch._dynamo.variables.streams import _setup_stream_event_cache"
                        )
                        code.writeline(
                            f"_setup_stream_event_cache({default_indices}, {new_indices}, {event_indices}, device_idx={self.device_idx})"
                        )
