...
class NVUniversalGemmKernel(Kernel):
    ...
    def render(self) -> str:
        ...
        disk_cache_config_key = f"_DISK_CACHE_CONFIG_KEY"
        ...
        with code.indent():
            ...
            with code.indent():
                for i, (param_name, _) in enumerate(self._template_input_args):
                    code.writeline(
                        f"{param_name} = torch.empty_strided("
                    )
                    with code.indent():
                        code.writeline(
                            f"tuple(precompile_shapes[{param_name!r}]),"
                        )
                        code.writeline(
                            f"tuple(precompile_strides[{param_name!r}]),"
                        )
                        code.writeline(
            ...
            with code.indent():
                ...
                code.writeline("'cutlass_api.providers.cutedsl.gemm.sm100_static_persistent',")
                code.writeline("'cutlass_api.providers.cutedsl.gemm.sm100_static_persistent_efc',")
                code.writeline("'cutlass_api.providers.cutedsl.gemm.sm100_dense_blockscaled_static_persistent',")
                code.writeline("'cutlass_api.providers.cutedsl.gemm.sm100_contiguous_offset_2d3d_dense_gemm',")
            code.writeline("]:")
            with code.indent():
                code.writeline("try:")
                with code.indent():
                ...
                with code.indent():
                    code.writeline("_patched_mods.append((_m, _m.get_max_active_clusters))")
                    code.writeline("_m.get_max_active_clusters = _mac_fn")
            code.writeline("try:")
            with code.indent():
                self._write_assign_call(
                    code, "kernel", "_lookup_gemm_kernel",
                    (kernel_name_var,),
                )
                ...
                code.writeline(f"input_tensors = {input_tensors_expr}")
                self._write_assign_call(
                    code, "args", "_create_gemm_arguments",
                    (
                        f'variant_name="{self.variant.name}"',
                        "input_tensors=input_tensors",
                        "out=out_ptr0",
                ...
                self._write_assign_call(
                    code, "cache_key", "_create_gemm_cache_key",
                    (
                        f'variant_name="{self.variant.name}"',
                        "input_tensors=input_tensors",
                        "out=out_ptr0",
